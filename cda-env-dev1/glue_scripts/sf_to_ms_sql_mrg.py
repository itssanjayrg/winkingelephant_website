...
from typing import Any, Dict, List, Optional, Tuple, Generator, NamedTuple
...
class BuildWhereClause:
    """
    Encapsulates the three override switches and produces
    the Snowflake WHERE clause on demand.
    """

    load_all_ids: bool  # FULL load
    load_all_incremental_ids: bool  # bootstrap incremental
    ids_to_load: str  # e.g. "12,34,54"

    @property
    def ids_sql_in(self) -> str:
        # Helper to format ids_to_load for SQL IN clause
        return ", ".join(i.strip() for i in self.ids_to_load.split(",") if i.strip())

    @property
    def has_valid_ids_to_load(self) -> bool:
        # Returns True if there is at least one valid ID
        return any(i.strip() for i in self.ids_to_load.split(","))

    def build_where(
        self, last_success_batch_id: int, is_override_tables_load: bool
    ) -> str:
        if is_override_tables_load:
            # If all override flags are missing/empty, fall back to regular incremental
            if (
                not self.load_all_ids
                and not self.load_all_incremental_ids
                and not self.has_valid_ids_to_load
            ):
                return f"WHERE BATCHID > {last_success_batch_id} AND gwcbi___operation != 0"
            if self.load_all_ids:
                return ""  # full load - ignores batchid & gwcbi___operation
            if self.load_all_incremental_ids:
                return (
                    "WHERE gwcbi___operation != 0"  # bootstrap incr - ignores batchid
                )
            if self.has_valid_ids_to_load:
                return f"WHERE ID IN ({self.ids_sql_in})"  # selective IDs - ignores batchid
        else:
            # regular incremental
            return f"WHERE BATCHID > {last_success_batch_id} AND gwcbi___operation != 0"
...
class Table:
    ...
    def set_sf_incremental_and_soft_delete_record_counts(
        self, cursor: SnowflakeCursor, is_override_tables_load: bool
    ) -> None:
        where_clause_sql = self.where_clause.build_where(
            last_success_batch_id=self.last_successful_sf_table_batch_id,
            is_override_tables_load=is_override_tables_load,
        )
        self.logger.info(
            f"Built WHERE clause for export count query: {where_clause_sql} for table {self.base_table_name}"
        )
    ...
    def copy_into_s3(
        ...
        self.logger.info(
            f"Built WHERE clause for unloading count query: {where_clause_sql} for table {self.base_table_name}"
        )
    ...
    def unload_sf_table_to_s3(
        self,
        snowflake_connection: SnowflakeConnection,
        is_override_tables_load: bool,
    ) -> None:
        self.logger.info(f"Starting unload for table: {self.base_table_name}")

        with snowflake_connection.cursor() as cursor:
            self.set_sf_incremental_and_soft_delete_record_counts(
                cursor=cursor, is_override_tables_load=is_override_tables_load
            )
    ...
    def process_records_for_deletion(
        self, df: DataFrame
    ) -> Tuple[DataFrame | None, DataFrame | None, DataFrame | None, int, int]:
        index_col = "ID"
        ...
        # need count of records read from s3 to spark for audit count validation
        df_incremental_rec_count = df_incremental.count()
        df_soft_delete_rec_count = df_soft_delete.count()
    ...
    def delete_ms_sql_records(
        ...
        try:
            ...
            self.logger.info(
                f"Deleted {deleted_count}/{to_delete_ids_count} {delete_type} records from table '{self.table_name}' using staging table."
            )
            ...
            self.logger.info(
                f"Successfully deleted '{self.table_name} table's records from '{del_stg_table_name}' table after inserting records into Delete Stage table and then deleting in '{self.table_name}'."
            )
    ...
    def validate_and_reconcile_data(
        ...
        self.load_audit_validation(
            df_incremental_rec_count=df_incremental_rec_count,
            df_soft_delete_rec_count=df_soft_delete_rec_count,
    ...
    def load_ms_sql_table(
        ...
        try:
            ...
            self.deleted_incremental_record_count = self.delete_ms_sql_records(
                df_del_ids=df_del_incremental_ids, delete_type="incremental_delete"
            )
            ...
            self.deleted_soft_delete_record_count = self.delete_ms_sql_records(
                df_del_ids=df_del_soft_delete_ids, delete_type="soft_delete"
            )
...
class Handler:
    ...
    def fetch_sf_tables(
        ...
        core_center: str,
        table_suffix: str,
        is_override_tables_load: bool = False,
        load_only_override_tables: bool = False,
    ) -> List[Table]:
        join_type = "INNER JOIN" if load_only_override_tables else "LEFT JOIN"
        self.logger.info(
            f"The load_only_override_tables flag is set to {load_only_override_tables} and the join type is {join_type}"
        )

        # Determine the query based on whether we are loading override tables or we are doing a normal load
        if not is_override_tables_load:
            query = f"""
                SELECT
                    CTL.TABLE_NAME,
                    LD_STS.BATCH_ID AS LATEST_BATCH_ID,
                    FALSE AS LOAD_ALL_IDS,
                    FALSE AS LOAD_ALL_INCREMENTAL_IDS,
                    '' AS IDS_TO_LOAD
            ...
            query = f"""
                SELECT
                    CTL.TABLE_NAME,
                    LD_STS.BATCH_ID                    AS LATEST_BATCH_ID,
                    COALESCE(OVRD.LOAD_ALL_IDS, FALSE) AS LOAD_ALL_IDS,
                    COALESCE(OVRD.LOAD_ALL_INCREMENTAL_IDS, FALSE) AS LOAD_ALL_INCREMENTAL_IDS,
                    COALESCE(OVRD.IDS_TO_LOAD, '') AS IDS_TO_LOAD
                FROM ETL_CTRL.JC_SF_TO_MSSQL_TABLES AS CTL
                    INNER JOIN ETL_CTRL.JC_MRG_{core_center.upper()}_LOAD_STATUS AS LD_STS
                        ON LOWER(CTL.TABLE_NAME) = LOWER(LD_STS.TABLE_NAME)
                    {join_type} ETL_CTRL.JC_SF_TO_MSSQL_OVERRIDE_TABLES OVRD
                        ON  LOWER(CTL.TABLE_NAME)  = LOWER(OVRD.TABLE_NAME)
                        AND LOWER(OVRD.CORE_CENTER)= LOWER('{core_center}')
                WHERE LD_STS.STATUS = 'completed'
            """
        # TODO need to remove the max(batchid) and group by confirm with its code developer
        # TODO to add logic to sort light tables first and heavy tables later.
        try:
            self.logger.info(
                f"Executing query to fetch tables from Snowflake that qualify for this batch load:\n {query}"
            )
            ...
            for (
                ...
                is_override_table = (
                    bool(load_all_ids)
                    or bool(load_all_incr_ids)
                    or (isinstance(ids_to_load, str) and ids_to_load.strip() != "")
                )

                # TODO discuss more in this with Payal and Sriram and oversee how to oversee override table conditions
                if latest_batch_id <= last_successful_sf_table_batch_id:
                    if not is_override_table:
                        self.logger.info(
                        ...
                        self.logger.info(
                            f"Not skipping {table_name} because it is an override table, even though latest_batch_id ({latest_batch_id}) <= last_successful_sf_table_batch_id ({last_successful_sf_table_batch_id})."
                        )

                ids_to_load = ", ".join(
                    ids.strip() for ids in (ids_to_load or "").split(",") if ids.strip()
                )

                where_clause = BuildWhereClause(
                    load_all_ids=bool(load_all_ids),
                    load_all_incremental_ids=bool(load_all_incr_ids),
                ...
                self.logger.info(
                    f"Adding table for processing: table_name='MRG_{core_center.upper()}.{table_name}', "
                    f"current_sf_table_batch_id={latest_batch_id}, "
                    f"last_successful_sf_table_batch_id={last_successful_sf_table_batch_id}, "
                    f"override_mode={'YES' if is_override_table else 'NO'}, "
                    f"override_flags=(load_all_ids={load_all_ids}, load_all_incr_ids={load_all_incr_ids}, ids_to_load='{ids_to_load}'), "
                    f"current_mssql_job_batch_id={self.current_mssql_job_batch_id}, "
                    f"where_clause={where_clause}"
                )

            self.logger.info(
                f"Total tables that qualified to process are : {len(tables)}"
            )
            return tables
            ...
            raise Exception("Error fetching table list") from exc

    def unload_sf_tables_to_s3(
        self, tables: List[Table], is_override_tables_load: bool
    ...
    def load_into_ms_sql(
        ...
        with ThreadPoolExecutor(max_workers=self.max_spark_workers) as executor:
            ...
            total_tables_to_load = len(futures)
            completed_count = 0
            failed_count = 0
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    completed_count += 1
                    self.logger.info(
                        f"Loaded table: {table.table_name} | "
                        f"Completed: {completed_count} | "
                        f"Failed: {failed_count} | "
                        f"Pending: {total_tables_to_load - (completed_count + failed_count)}"
                    )
                except Exception as exc:
                    failed_tables.append((table, exc))
                    failed_count += 1
                    self.logger.error(
                        f"Failed to load table: {table.table_name} | "
                        f"Completed: {completed_count} | "
                        f"Failed: {failed_count} | "
                        f"Pending: {total_tables_to_load - (completed_count + failed_count)}"
                    )
        ...
        self.logger.info(
            f"Number of tables successfully loaded into mssql: {len(loaded_tables)}/{total_tables_to_load}"
        )
        self.logger.info(
            f"Number of tables failed to load into mssql: {failed_count}/{total_tables_to_load}"
        )
...
def main(
    ...
    tables = handler.fetch_sf_tables(
        ...
        table_suffix=table_suffix,
        is_override_tables_load=is_override_tables_load,
        load_only_override_tables=load_only_override_tables,
    )
    ...
    handler.logger.info(
        f"is_override_tables_load is set to {is_override_tables_load}. "
        f"Tables will be unloaded and loaded using tables present in Control table: "
        f"{'JC_SF_TO_MSSQL and JC_SF_TO_MSSQL_OVERRIDE' if is_override_tables_load else 'only JC_SF_TO_MSSQL'}."
    )

    # 1.3 Unload incremental records from snowflake
    failed_tables = handler.unload_sf_tables_to_s3(
        tables=tables, is_override_tables_load=is_override_tables_load
    )
    ...
    # Phase 2: Load into MS SQL Server
...
if __name__ == "__main__" and IS_GLUE_JOB:
    ...
    #########################################################################################
    # Read the comments before you make changes to the load_only_override_tables variable
    # This flag should be set to True only if you want to process ONLY tables present in the override table (JC_SF_T0_MSSQL_OVERRIDE)
    # TODO: add documentation for this

    # This is applied in AWS Glue
    load_only_override_tables = False

    #########################################################################################
    # Read the comments before you make changes to the is_override_tables_load variable
    # This flag should be set to True only if we need to override loading few tables
    # The condition are highlighted in the document - cda/docs/snowflake_to_onprem_logic.md
    #
    # This is applied in AWS Glue
    ...
    # Read the comments before you make changes to the load_only_override_tables variable
    # This flag should be set to True only if you want to process ONLY tables present in the override table (JC_SF_T0_MSSQL_OVERRIDE)
    # this also ignores skipping of tables based on the last successful batch id

    # This is applied in Local
    load_only_override_tables = False
